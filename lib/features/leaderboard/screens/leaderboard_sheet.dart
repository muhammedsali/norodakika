...
final globalLeaderboardProvider =
  ...
  final firebaseUser = ref.watch(currentUserProvider).value;
  ...
  for (var data in usersData) {
    ...
    users.add(
        LeaderboardUser(id: uid, name: name, score: score, avatarUrl: avatar));
  }
  ...
  if (!amIInList && myUid != 'me') {
    ...
    final userName = customName ??
        (firebaseUser?.displayName ??
            (firebaseUser?.email?.split('@')[0] ?? fallBackName));

    final myStats = ref.watch(userStatsProvider).value ?? {};
    ...
    users.add(LeaderboardUser(
        ...
        name: userName,
        score: myScore,
        avatarUrl: 'assets/icons/app_icon.png'));
  }
              ...
              return ClipRRect(
                ...
                child: BackdropFilter(
                  ...
                  child: dbUsersAsync.when(
                    loading: () => Container(
                        color: sheetColor.withValues(
                            alpha: isDarkMode ? 0.85 : 0.90),
                    ...
                    error: (err, stack) => Container(
                        ...
                        child: Center(
                            child: Text(
                                'Veritabanına bağlanılamadı. / Connection failed.\n\n$err',
                                textAlign: TextAlign.center,
                                style: TextStyle(color: textColor)))),
                    data: (users) {
                      final userRank =
                          users.indexWhere((u) => u.id == myUid) + 1;
